---
title: 3. Local Setup for Testing
description: Setup Postgres via Docker Compose with OLake UI
sidebar_position: 3
---
...
:::info
Now that you have configured the local database, you can either follow the [getting-started guide](../../../getting-started/postgres#step-2-generate-a-streams-file) to start syncing data or review the broader [PostgreSQL connector overview](/docs/connectors/postgres/overview) to understand the complete ELT flow with OLake.
:::
...
-- Disable timeout on sender (to avoid snapshot disconnects)
...
For stricter security, add explicit read permissions:

```sql
-- Example for the "public" schema in database main
GRANT CONNECT ON DATABASE main TO olake_user;
GRANT USAGE ON SCHEMA public TO olake_user;
GRANT SELECT ON ALL TABLES IN SCHEMA public TO olake_user;
-- Ensure future tables are covered automatically
ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT SELECT ON TABLES TO olake_user;
``` 
...
SELECT * FROM pg_create_logical_replication_slot('your_olake_slot_name', 'wal2json'); 
```

**Example with common slot names used in local setup:**
```sql
-- If your OLake source config uses "postgres_slot" (as shown in docker-compose example)
